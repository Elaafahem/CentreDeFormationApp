...
public class StatisticsService {
        ...
        public Map<String, Object> getFilteredStats(Long coursId, Long formateurId, String etudiantEmail) {
                ...
                if (etudiantEmail != null && !etudiantEmail.isEmpty()) {
                        inscriptions = inscriptions.stream()
                                        .filter(i -> i.getEtudiant() != null
                                                        && etudiantEmail.equalsIgnoreCase(i.getEtudiant().getEmail()))
                                        .collect(Collectors.toList());

                        Set<Long> courseIds = inscriptions.stream().map(i -> i.getCours().getId())
                                        .collect(Collectors.toSet());
                        courses = courses.stream().filter(c -> courseIds.contains(c.getId()))
                                        .collect(Collectors.toList());

                        grades = grades.stream()
                                        .filter(n -> n.getInscription() != null
                                                        && n.getInscription().getEtudiant() != null
                                                        && etudiantEmail.equalsIgnoreCase(
                                                                        n.getInscription().getEtudiant().getEmail()))
                                        .collect(Collectors.toList());
                ...
                if (coursId != null) {
                        ...
                        students = students.stream().filter(s -> studentIds.contains(s.getId()))
                                        .collect(Collectors.toList());
                        courses = courses.stream().filter(c -> c.getId().equals(finalCoursId))
                                        .collect(Collectors.toList());
        ...
        public List<Map<String, Object>> getEnrollmentsByMonth(Long coursId, Long formateurId, String etudiantEmail) {
                ...
                Map<Integer, Long> enrollmentsByMonth = inscriptions.stream()
                                .collect(Collectors.groupingBy(
                                                i -> i.getDateInscription().getMonthValue() - 1, // 0-based index for
                                                                                                 // array
                                                Collectors.counting()));
        ...
        public List<Map<String, Object>> getPerformanceDistribution(Long coursId, Long formateurId,
        ...
        public List<Map<String, Object>> getRecentActivity(Long coursId, Long formateurId, String etudiantEmail) {
                ...
                notes.stream().sorted((a, b) -> b.getId().compareTo(a.getId())).limit(2).forEach(n -> {
                        ...
                        act.put("description", "Note de " + n.getValeur() + " pour "
                                        + (n.getInscription().getEtudiant() != null
                                                        ? n.getInscription().getEtudiant().getNom()
                                                        : "Etudiant"));
                        act.put("time", "Récent");
                        activities.add(act);
                });
